...
void md_cell::verlet(){
    for(std::size_t i = 0; i < np; i++){
        for(std::size_t j = 0; j < 3; j++){
            double xx = sites_cart[i][j] + vel[i][j]*dt + 0.5*SQR(dt)*force[i][j];
            xx -= box*floor(xx/box);
            double delta_x = xx - sites_cart[i][j];
            if(delta_x > box/2.) delta_x -= box;
            if(delta_x < -box/2.) delta_x += box;
            trace[i][j] += delta_x;
//            trace[i][j] += (sites_cart[i][j] - xm[i][j]);
            sites_cart[i][j] = xx;
            vel[i][j] += force[i][j]*dt_2;
        }
    }
}
...
void md_cell::init_ver(double T){
...
//        cout << i << " " << trace2 << " " << Tc <<  endl;
//        cout << "ok" << endl;
        if( i%nsampl == 0 && i > 100000){
            md.ComputePCF();
            double Tc = 2.*md.KE/(3.*md.np);
...
//            cout << Tc << " " << pres << endl;
        ...
        if( i < ncycl ){
            ...
            double sf = sqrt(3.*md.np*Tb/(2.*md.KE));
            for(std::size_t j = 0; j < md.np; j++){
